...
const InternationalProjectList = () => {
  ...
  const sortedNews = [...internationalProjects].sort((a, b) => new Date(b.date) - new Date(a.date));
        ...
        <div className="container mx-auto px-4">
          <section className="max-w-5xl mx-auto rounded-2xl bg-white/95 border border-[#E5C0C8]/60 shadow-md p-6 md:p-10">
            <div className="pb-4 border-b border-[#E5C0C8]/60">
              <h1 className="text-2xl md:text-3xl font-bold text-gray-900">国际宫健康管理工程</h1>
              {keyword && (
            ...
            <div className="mt-4 divide-y divide-[#E5C0C8]/60">
              {pagedNews.length === 0 && (
                <div className="py-10 text-center text-gray-500">暂无相关内容，请尝试其他关键词。</div>
              )}
              {pagedNews.map((item) => {
                    ...
                    <div className="flex flex-col md:flex-row md:items-start gap-4 md:gap-6">
                      ...
                      <div className="min-w-0 flex-1">
                        <h2 className="text-lg md:text-xl font-semibold text-gray-900 leading-8">
                          <Link to={`/projects/${item.slug}`} className="hover:text-[#194F92]">
                            {item.title}
                          </Link>
                        </h2>
                        ...
                        <div className="mt-3 text-sm text-gray-500">
                          发布时间：{item.date}
                          <Link to={`/projects/${item.slug}`} className="ml-5 text-[#194F92] hover:underline">
                            详情
                          </Link>
                        </div>
                      </div>
...
export default InternationalProjectList;
